...
public:
  ...
  /// 创建标准的 PLOS 风格序言：保存 FP/LR 并更新 SP
  /// StpInst: 将被填充为 stp x29, x30, [sp, #-16]!
  /// MovInst: 将被填充为 mov x29, sp
  virtual void createOutliningPrologue(MCInst &StpInst, MCInst &MovInst) const {
    llvm_unreachable("not implemented for this target");
  }

  /// 创建标准的 PLOS 风格结语：恢复 FP/LR 并释放 SP
  /// LdpInst: 将被填充为 ldp x29, x30, [sp], #16
  virtual void createOutliningEpilogue(MCInst &LdpInst) const {
    llvm_unreachable("not implemented for this target");
  }
