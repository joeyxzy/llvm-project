...
// 定义打印 Pass 结果的开关 (调试用)
static cl::opt<bool>
    PrintOutlining("print-outlining",
                   cl::desc("print functions after aarch64 outlining"),
                   cl::Hidden, 
...
Error BinaryFunctionPassManager::runAllPasses(BinaryContext &BC) {
  ...
  // 只在 AArch64 架构下注册，并且只有当命令行开启了 EnableOutlining 时才运行
  // if (BC.isAArch64()) {
  //   Manager.registerPass(
  //       std::make_unique<AArch64Outlining>(opts::PrintOutlining),
  ...
  if (BC.isAArch64()) {
    ...
    Manager.registerPass(std::make_unique<AArch64Outlining>(opts::PrintOutlining),opts::EnableOutlining);
  }
