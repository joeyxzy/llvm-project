...
bool hasRefToStackOrFramePointer(const MCInst &Inst) const override {
  ...
  for (const MCOperand &Op : Inst) {
    if (Op.isReg()) {
      ...
      // 必须包含 AArch64 的寄存器定义头文件
      // #include "MCTargetDesc/AArch64MCTargetDesc.h" (如果在 LLVM 树内)
      // 或者直接用 magic number (不推荐)，或者用名字
      
      // 方法 A: 如果能引用 AArch64::SP
      if (Reg == AArch64::SP || Reg == AArch64::WSP ||
          Reg == AArch64::FP || Reg == AArch64::W29) { // 加上 FP !
        return true;
      }
      
      // 方法 B: 字符串回退方案 (如果找不到枚举定义)
      // StringRef Name = BC.MRI->getName(Reg);
      // if (Name.equals_insensitive("sp") || Name.equals_insensitive("wsp") ||
      //     Name.equals_insensitive("fp") || Name.equals_insensitive("w29") || 
      //     Name.equals_insensitive("x29")) 
      //   return true;
    }
  }
  return false;
  ...
  void createOutliningPrologue(MCInst &StpInst, MCInst &MovInst) const override {
    // 1. 构建 stp x29, x30, [sp, #-16]!
    StpInst.setOpcode(AArch64::STPXpre);
    StpInst.clear();
    StpInst.addOperand(MCOperand::createReg(AArch64::SP)); // Writeback SP
    StpInst.addOperand(MCOperand::createReg(AArch64::FP)); // x29
    StpInst.addOperand(MCOperand::createReg(AArch64::LR)); // x30
    StpInst.addOperand(MCOperand::createReg(AArch64::SP)); // Base SP
    StpInst.addOperand(MCOperand::createImm(-2));          // Offset -16 (-2 * 8)
  
    // 2. 构建 mov x29, sp (即 add x29, sp, #0)
    MovInst.setOpcode(AArch64::ADDXri);
    MovInst.clear();
    MovInst.addOperand(MCOperand::createReg(AArch64::FP)); // Dest
    MovInst.addOperand(MCOperand::createReg(AArch64::SP)); // Src
    MovInst.addOperand(MCOperand::createImm(0));           // Imm
    MovInst.addOperand(MCOperand::createImm(0));           // Shift
  }
  
  void createOutliningEpilogue(MCInst &LdpInst) const override {
    // 3. 构建 ldp x29, x30, [sp], #16
    LdpInst.setOpcode(AArch64::LDPXpost);
    LdpInst.clear();
    LdpInst.addOperand(MCOperand::createReg(AArch64::SP)); // Writeback SP
    LdpInst.addOperand(MCOperand::createReg(AArch64::FP)); // x29
    LdpInst.addOperand(MCOperand::createReg(AArch64::LR)); // x30
    LdpInst.addOperand(MCOperand::createReg(AArch64::SP)); // Base SP
    LdpInst.addOperand(MCOperand::createImm(2));           // Offset 16 (2 * 8)
  }
