...
bool hasIllegalSymbolOperand(const MCInst &Inst) {
  for (const MCOperand &Op : Inst) {
    ...
    if (auto *SymExpr = dyn_cast<MCSymbolRefExpr>(Expr)) {
      ...
      // LLVM22 不允许直接访问 protected 成员
      // 用符号名字或者 BOLT 提供的接口判断
      StringRef Name = Sym.getName();
      if (Name.empty()) return true;  // 不可识别符号
      if (Name.starts_with(".L")) continue; // 局部符号一般没问题
      // 这里判断是不是全局或外部符号
      if (Sym.isTemporary() || Sym.isCommon()) continue; // 局部或 BSS
      return true; // 其它符号当非法
    }
  }
  return false;
...
createOutlinedFunction(BinaryContext &BC, const Candidate &C, int ID) {
  ...
  if (!OutBF) {
    errs() << "BOLT-ERROR: Failed to create injected function " << FuncName << "\n";
    return nullptr;
  }
  ...
  // 4. 添加 Prologue/Epilogue/Return
  auto &Builder = BC.MIB;
  MCInst Prologue_stp, Prologue_mv, Epilogue;
  Builder->createOutliningPrologue(Prologue_stp,Prologue_mv);
  Builder->createOutliningEpilogue(Epilogue);
  // Builder->createStorePair(Prologue, Builder->getFramePointer(), Builder->getLinkReg());
  // Builder->createLoadPair(Epilogue, Builder->getFramePointer(), Builder->getLinkReg());
  MCInst Return;
  ...
  for (uint64_t k = 0; k < C.Length; ++k) {
    ...
    const MCInst &OrigInst = HI.BB->getInstructionAtIndex(HI.BBIndex);
    MCInst NewInst = OrigInst;
    if (BC.MIB->mayLoad(OrigInst) || BC.MIB->mayStore(OrigInst)){
      //BC.MIB->fixOutlinedStackOffset(NewInst);
    }
    BB->insertInstruction(BB->end(), NewInst);
  }

  // 6. 插入 Prologue/Epilogue/Return
  // BB->insertInstruction(BB->begin(), Prologue_mv);
  // BB->insertInstruction(BB->begin(), Prologue_stp);
  // BB->insertInstruction(BB->end(), Epilogue);
  BB->insertInstruction(BB->end(), Return);
  
  outs() << "BOLT-INFO: Created " << FuncName << "\n";
  return OutBF;
}
...
void mapInstructions(BinaryContext &BC) {
  ...
  for (BinaryFunction *BF : AllFunctions) {
    ...
    for (BinaryBasicBlock &BB : Function) {
      ...
      for (MCInst &Inst : BB) {
        // 过滤
        if (BC.MIB->isPseudo(Inst) || BC.MIB->isCFI(Inst) || Inst.getOpcode() <= 100) {
          BBIdx++; continue;
        }
        ...
        if (BC.MIB->isBranch(Inst) || BC.MIB->isTerminator(Inst) || 
            hasIllegalSymbolOperand(Inst) || BC.MIB->hasPCRelOperand(Inst)) {
